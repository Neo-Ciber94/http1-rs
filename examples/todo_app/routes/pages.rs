    ...
    Scope::new()
        .get("/", |State(db): State<KeyValueDatabase>, auth: AuthenticatedUser, alert: Option<AlertProps>| -> Result<HTMLElement, ErrorResponse> {
            ...
            Ok(html::html(|| {
                ...
                html::body(|| {
                    Layout(LayoutProps { auth: Some(auth), alert, class: None }, || {
                        ...
                        html::div(|| {
                            ...
                            todos.iter().for_each(|todo| {
                                html::div(|| {
                                    ...
                                    html::a(|| {
                                        html::attr("href", format!("/todos/{}", todo.id));
                                        html::class("w-full");
        ...
        .get("/create", | auth: AuthenticatedUser, alert: Option<AlertProps>| {
            html::html(|| {
                ...
                html::body(|| {
                    Layout(LayoutProps { auth: Some(auth), alert, class: Some(String::from("flex-grow")) }, || {
            ...
            Ok(html::html(|| {
                ...
                html::body(|| {
                    Layout(LayoutProps { auth: Some(auth), alert, class: Some(String::from("flex-grow")) }, || {
                        ...
                        html::div(|| {
                            html::class("bg-white p-8 rounded-lg shadow-lg w-full max-w-md text-center");
        
                            html::h1(|| {
                                html::content(&todo.title);
                                html::class(format!(
                                    "text-xl font-bold mb-4 {} {}",
                                    if todo.is_done { "text-gray-400 line-through italic" } else { "text-blue-600" },
                                    if todo.is_done { "opacity-50" } else { "" }
                                ));
                            });
                            ...
                            if let Some(desc) = &todo.description {
                                html::p(|| {
                                    html::content(desc);
                                    html::class(format!("text-gray-600 mb-6 {}", if todo.is_done { "text-gray-400 line-through italic opacity-50"} else { "" }));
                                });
                            }
        
                            html::div(|| {
                                html::class("flex justify-center gap-2 w-full mt-6");
                                ...
                                html::form(|| {
                                    ...
                                    html::class("contents");
                                    html::button(|| {
                                        html::content(if todo.is_done { "Completed" } else { "Pending" });
                                        html::class(format!(
                                            "px-4 py-2 rounded-md text-white font-semibold w-full {}",
                                            if todo.is_done { "bg-green-500" } else { "bg-amber-500" }
                                        ));
                                    });
                                });
                                ...
                                html::a(|| {
                                    html::attr("href", format!("/todos/edit/{}", todo.id));
                                    html::class("px-4 py-2 w-full rounded-md bg-blue-500 text-white font-semibold text-center");
                                    html::content("Edit");
                                });
                            });
                        });
    ...
    Scope::new()
        .get("/login", |auth: Option<AuthenticatedUser>, alert: Option<AlertProps>| -> Result<HTMLElement, Redirect> {
            ...
            Ok(html::html(|| {
                ...
                html::body(|| {
                    Layout(LayoutProps { auth, alert, class: Some(String::from("flex-grow")) }, || {
                        html::div(|| {
                            html::class("bg-white p-8 rounded shadow-lg w-full max-w-md");
